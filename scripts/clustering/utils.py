...
def build_global_leaderboard(datasets_map):
    ...
    for emb, X in datasets_map.items():
        # KMeans
        km_best = gridsearch_kmeans_params(X, ks=range(2, 15), n_inits=(10, 20))["params"]
        if km_best:
        ...
        # DBSCAN
        db_best = gridsearch_dbscan(X)  # returns dict with params/metrics/labels
...
def _score_internal(m):
    ...
    # primary: silhouette
    # secondary: CH
    # tertiary: invert DB (smaller is better)
...
def plot_clusters_target_3d(X, cluster_labels, y_target, title="3D Clusters vs DEATH_EVENT"):
    ...
    # Colors for DEATH_EVENT
    color_map = {0: "#0077BB", 1: "#EE7733"}  # blue/orange
    ...
    # Add one trace per cluster-target combo
            ...
            if np.any(mask):
                fig.add_trace(
                    go.Scatter3d(
                        ...
                        hovertemplate=(
                            f"Cluster: {c}<br>DEATH_EVENT: {'Death' if t else 'No'}"
...
def _plot_curves(df: pd.DataFrame, title: str, include_inertia: bool = False) -> None:
    """Plot metric curves vs k. No specific colors/styles set."""
...
def select_k_kmeans(
    ...
    """
    Sweep KMeans k, compute metrics: inertia (↓), silhouette (↑), CH (↑), DB (↓).
    Recommends k by each metric.
    """
    rows = []
    for k in k_range:
        ...
        row = {"k": k, "inertia": float(km.inertia_)}
        row.update(met)
        rows.append(row)
    ...
    _plot_curves(df, title="KMeans model selection", include_inertia=True)
...
def select_k_agglomerative(
    ...
    """
    Sweep Agglomerative k for a given linkage/metric, compute: silhouette (↑), CH (↑), DB (↓).

    Notes:
      - 'ward' requires Euclidean and ignores 'metric'; it minimizes within-cluster variance.
      - For non-ward linkages, you may set metric="euclidean" explicitly (default in sklearn).
    """
    ...
    for k in k_range:
        if linkage == "ward":
            ...
            agg = AgglomerativeClustering(n_clusters=k, linkage=linkage, metric=metric or "euclidean")
        labels = agg.fit_predict(X)
        met = _compute_internal_metrics(X, labels)
        row = {"k": k}
        row.update(met)
    ...
    title = f"Agglomerative model selection (linkage={linkage}{', metric='+metric if metric else ''})"
    _plot_curves(df, title=title, include_inertia=False)
